...
class fish():
    def __init__(self, root, is_train=True, transform=None):
        ...
        if self.is_train:
            for cla in self.self.classes:
                for file_name in os.listdir(os.path.join(self.root, 'train', cla)):
                    train_img = scipy.misc.imread(os.path.join(self.root, 'train', cla, file_name))
                    self.img_list.append(train_img)
                    self.img_name_list.append(file_name)
                    self.img_label_list.append(cla)

        if not self.is_train:
            for file_name in os.listdir(os.path.join(self.root, 'test_stg1')):
                train_img = scipy.misc.imread(os.path.join(self.root, 'train', file_name))
                self.img_list.append(train_img)
                self.img_name_list.append(file_name)
                self.img_label_list.append(cla)
    ...
    def __getitem__(self, index):
        if self.is_train:
            img, target, imgname = self.img_list[index], self.img_label_list[index], self.img_name_list[index]
            if len(img.shape) == 2:
                img = np.stack([img] * 3, 2)
            img = Image.fromarray(img, mode='RGB')
            if self.transform is not None:
